<?php 
	...
	class CrudUsuario{
		...
		public function obtenerUsuario($nombre, $clave){
			$db=Db::conectar();
			$select=$db->prepare('SELECT * FROM USUARIOS WHERE nombre=:NombreC');//AND clave=:clave
			$select->bindValue('nombre',$NombreC);
			$select->execute();
		...
		public function buscarUsuario($nombre){
			$db=Db::conectar();
			$select=$db->prepare('SELECT * FROM clientes WHERE nombre=:NombreC');
			$select->bindValue('nombre',$NombreC);
			$select->execute();
			$registro=$select->fetch();
			if($registro['Id']!=NULL){
